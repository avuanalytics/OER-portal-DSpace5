...
public class ItemAdapter extends AbstractAdapter
{
    ...
    protected void renderDescriptiveSection() throws WingException, SAXException, CrosswalkException, IOException, SQLException
    {
        ...
        Set<String> htmlEnabledFields = getHtmlEnabledMetadataFields();
        ...
        if(dmdTypes.size() == 0 || dmdTypes.contains("DIM"))
        {
            ...
            for (Metadatum dcv : dcvs)
            {
                if (!MetadataExposure.isHidden(context, dcv.schema, dcv.element, dcv.qualifier))
                {
                    ...
                    if (htmlEnabledFields.contains(dcv.getField())) {
                        SimpleHTMLFragment valueFragment = new SimpleHTMLFragment(new WingContext(), true, dcv.value);
                        valueFragment.toSAX(contentHandler, lexicalHandler, namespaces);
                    } else {
    ...
    private Set<String> getHtmlEnabledMetadataFields() {
        String fields = StringUtils.trimToEmpty(ConfigurationManager.getProperty("xmlui.html.metadataFields"));
        HashSet output = new HashSet<>();
        for (String field : fields.split(",")) {
            output.add(StringUtils.trimToEmpty(field));
        }
        return output;
    }
