...
<xsl:stylesheet
            ...
            <div class="row">
                <div class="col-sm-4">
                    ...
                    <xsl:call-template name="itemSummaryView-DIM-generic">
                        ...
                        <xsl:with-param name="qualifier1">author</xsl:with-param>
                    </xsl:call-template>
                ...
                <div class="col-sm-8">
                    ...
                    <xsl:call-template name="itemSummaryView-DIM-generic">
                        ...
                        <xsl:with-param name="qualifier1">abstract</xsl:with-param>
                    </xsl:call-template>
                    <xsl:call-template name="itemSummaryView-DIM-generic">
                        <xsl:with-param name="schema">dc</xsl:with-param>
                        <xsl:with-param name="element">identifier</xsl:with-param>
                        <xsl:with-param name="qualifier1">uri</xsl:with-param>
                    </xsl:call-template>
            ...
            <div>
                <xsl:attribute name="class">
                    <xsl:choose>
                        ...
                        <xsl:otherwise>
                            <xsl:text>simple-item-view-</xsl:text>
                            <xsl:value-of select="$schema"/>
                            <xsl:text>-</xsl:text>
                            <xsl:value-of select="$element"/>

                            <xsl:if test="not( $noqualifier and string-length($qualifier1) = 0)">
                                <xsl:text>-</xsl:text>
                                <xsl:value-of
                                        select="$qualifier1"/>
                            </xsl:if>
                            <xsl:text> item-page-field-wrapper table</xsl:text>
                        </xsl:otherwise>
                    </xsl:choose>
                </xsl:attribute>
                <xsl:if test="$header">
                    <h5>
                        <xsl:choose>
                            <xsl:when test="$noqualifier and string-length($qualifier1) = 0">
                                <i18n:text>xmlui.dri2xhtml.METS-1.0.item-<xsl:value-of
                                        select="$element"/>
                                </i18n:text>
                            </xsl:when>
                            <xsl:otherwise>
    ...
    <xsl:template name="toURL">
        <xsl:choose>
            <xsl:when test="text() and starts-with(text(), 'http')">
